...
const seedDatabase = async () => {
  try {
    ...
    // 3. CRÉATION DES CATÉGORIES (Échiquier, Pièces d'échec, Accessoires)
    ...
    console.log(
      "✅ Catégories créées : Échiquier, Pièces d'échec, Accessoires."
    );

    // Map pour retrouver facilement les IDs
    const catPieces = createdCategories.find(
      (c) => c.name === "Pièces d'échec"
    );
    const catEchiquier = createdCategories.find((c) => c.name === "Échiquier");
